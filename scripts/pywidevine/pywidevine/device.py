...
class Device:
    ...
    def from_dir(cls, dir: Union[Path, str]) -> Device:
        """Load a WVD device from a directory containing wv.json, device_private_key, and device_client_id_blob."""
        try:
            with open(os.path.join(dir, "wv.json")) as fd:
                config = json.load(fd)
        except FileNotFoundError:
            raise FileNotFoundError("wv.json file is required")

        try:
            with open(os.path.join(dir, "device_private_key"), "rb") as fd:
                private_key = fd.read()
        except FileNotFoundError:
            private_key = None

        with open(os.path.join(dir, "device_client_id_blob"), "rb") as fd:
            client_id = fd.read()

        try:
            with open(os.path.join(dir, "device_vmp_blob"), "rb") as fd:
                vmp = fd.read()
        except FileNotFoundError:
            vmp = None

        return cls(
            type_=getattr(DeviceTypes, config["session_id_type"].upper()),
            security_level=int(config["security_level"]),
            flags={
                "send_key_control_nonce": config.get(
                    "send_key_control_nonce",
                    config["session_id_type"] == "android"
                ),
            },
            private_key=private_key,
            client_id=client_id,
