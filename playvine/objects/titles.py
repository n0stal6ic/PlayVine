...
from playvine.objects.tracks import Tracks
...
class Title:
    ...
    def parse_filename(self, media_info=None, folder=False):
        from playvine.config import config
        ...
        if video_track:
            ...
            if video_track.width == 1248:  # non-standard 1248x520 widescreen
                quality = 720
        ...
        video = ""
        if video_track:
            hdr_commercial = video_track.hdr_format_commercial
            if hdr_commercial:
                if (video_track.hdr_format or "").startswith("Dolby Vision"):
                    if (hdr_commercial) != "Dolby Vision":
                        video += f" DV {DYNAMIC_RANGE_MAP.get(hdr_commercial)} "
                else:
                    video += f" {DYNAMIC_RANGE_MAP.get(hdr_commercial)} "
